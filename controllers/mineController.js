...
export const getMineById = catchAsyncError(async (req, res, next) => {
  const mine = await Mine.findById(req.params.id).populate("owner_id", "name phone").populate("materials").populate("assigned_trucks");
...
export const getGlobalMineAndMaterialCount = catchAsyncError(async (req, res, next) => {
  const [mineCount, materialCount] = await Promise.all([Mine.countDocuments(), Material.countDocuments()]);
...
export const updateMine = catchAsyncError(async (req, res, next) => {
  ...
  if (!mine) return next(new ErrorHandler("Mine not found", 404));
  if (mine.owner_id.toString() !== req.user.id) return next(new ErrorHandler("You can only update your own mines", 403));
...
export const deleteMine = catchAsyncError(async (req, res, next) => {
  ...
  if (mine.owner_id.toString() !== req.user.id && req.user.role !== "admin") {
    return next(new ErrorHandler("You can only delete your own mines or be an admin", 403));
  }
