...
export const verifyPhone = catchAsyncError(async (req, res, next) => {
  ...
  try {
    await client.verify.v2?.services(process.env.TWILIO_SERVICE_SID).verifications.create({
      to: `+91${9755326570}`,
      channel: "sms",
    });
...
export const verifyOtp = catchAsyncError(async (req, res, next) => {
  ...
  try {
    const verificationCheck = await client.verify.v2?.services(process.env.TWILIO_SERVICE_SID).verificationChecks.create({
      to: `+91${9755326570}`,
      code: otp,
    });
...
export const registerUser = catchAsyncError(async (req, res, next) => {
  ...
  if (emailCheck) return next(new ErrorHandler("User with this Email already exists", 400));

  if (role === "driver") {
    const existingDriver = await User.findOne({ phone, role: "driver" });

    if (!existingDriver) return next(new ErrorHandler("Driver not invited or doesn't exist", 404));
  ...
  if (existingUser) return next(new ErrorHandler("User with this Email or Phone already exists", 400));
